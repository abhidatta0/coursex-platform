...
export const uploadSingleMedia = async (formData: FormData) => {
    const {data} = await apiClient.post<MediaUploadResponse>(`${BASE_MEDIA_UPLOAD_URL}/upload`,formData,{
        headers:{
...
export const deleteSingleMediaByMetadata = async (publicId: string) => {
    const {data} = await apiClient.post<string>(`${BASE_MEDIA_UPLOAD_URL}/delete`,{publicId});
    return data;
};